...
func getRouter() (router *gin.Engine, err error) {
	defer func() {
	...
	router.POST("/auth/realms/master/protocol/openid-connect/token", func(gc *gin.Context) {
		gc.Header("Content-Type", "application/json; charset=utf-8")
		username := gc.Request.FormValue("username")
		var token string
		if gc.Request.FormValue("grant_type") == "password" && username == "" {
			gc.String(http.StatusBadRequest, "missing user name")
			return
		}
		if username == "admin" {
		...
		gc.JSON(http.StatusOK, security.OpenidToken{
			AccessToken:      token,
	...
	router.GET("/auth/admin/realms/master/users", func(gc *gin.Context) {
		username := gc.Query("username")
		gc.JSON(http.StatusOK, []security.UserRepresentation{{
			Id:   username,
			Name: username,
		}})
	})

	router.GET("/auth/admin/realms/master/users/:userid/role-mappings/realm", func(gc *gin.Context) {
		if gc.Param("userid") == "admin" {
			gc.JSON(http.StatusOK, []security.RoleMapping{{Name: "admin"}})
		} else {
			gc.JSON(http.StatusOK, []security.RoleMapping{{Name: "test-group"}})
		}
	})
